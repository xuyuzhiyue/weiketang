...
public class RoomServiceImp implements RoomService {
	...
	public void delete(Integer uuid) {
		sessionFactory.getCurrentSession().delete(
				sessionFactory.getCurrentSession().load(Room.class, uuid));
	}

	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public Room find(String uuid) {
		return (Room) sessionFactory.getCurrentSession().get(Room.class, Integer.parseInt(uuid));
	...
	public Room find(Room model) {
		try {
			List<Room> list = getHibernateTemplate().findByExample(model);
			if (list.size() > 0) {
				return list.get(0);
			}
	...
	public Page findByPage(Page page, Map paramsMap) {
		...
		sb.append("from Room u where 1=1");
